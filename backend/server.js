...
// API Endpoints - Register ALL API routes with /api/ prefix first
...
// API Endpoints - Legacy non-prefixed API routes (keep for backward compatibility)
...
// MODIFIED: This is the key change - Σερβίρισμα του React app για όλα τα άλλα routes
app.get('*', (req, res) => {
  // Only check for API endpoints that don't exist (keep only /api/ check)
